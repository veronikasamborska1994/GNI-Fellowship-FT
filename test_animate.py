...
def update_plot(attr, old, new):
    ...
    layout = column(p, widgetbox([slider,button]), widgetbox(select)) #sizing_mode='scale_width')
...
def animate_map():
    ...
    if button.label == '► Play':
        button.label = '❚❚ Pause'
        callback_id = curdoc().add_periodic_callback(animate_map_update, 2000)
    else:
        button.label = '► Play'
        curdoc().remove_periodic_callback(callback_id)

button = Button(label='► Play', width=60)
button.on_click(animate_map)
